        ...
        try {
            ...
            for(let key in self.allRoom){
                ...
                if((now - room.createTime >= outTime && room.status <= 1) || (now - room.createTime >= outTime2)){
                    ...
                    if(room.status <= 1 && room.roomType == 2){
                        let cardNum = room.roundCount == 8 ? 4 : 8;
...
RoomManager.prototype.getGameResultList = async function(uid,page){
    ...
    let count = 10;
    let results = await gameResult.find({$or : [obj , { 'result.ownerUid' : uid}],'result.createTime' : { $gte : t} }).sort({'result.createTime' : -1}).limit(10).skip(page * count);
    let arr = [];
